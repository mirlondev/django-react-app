...
const UserDetailPage = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      setLoading(true);
      const response = await usersAPI.getById(id); // <--- on utilise l'id de l'URL
  ...
  const fetchUserRatings = async () => {
    try {
      const response = await ratingsAPI.getUserRatings(id); // <--- passe l'ID ici
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (passwordData.new_password !== passwordData.confirm_password) {
      setMessage({ type: "id: string | undefinederror", text: "New passwords do not match" });
      return;
    }
